...
export class Battle {
    ...
    matching(p, r) { const condition = r.condition_type === "if_able" ? true : r.condition_type === "card_in_hand" ? p.hand.some(c => c.definition.card_id === r.condition_card_id) : r.condition_type === "own_victory_points_at_most" ? p.points <= (r.victory_points_threshold ?? 0) : r.condition_type === "opponent_victory_points_at_most" ? this.opponent(p).points <= (r.victory_points_threshold ?? 0) : r.condition_type === "opponent_sync_at_most" ? this.opponent(p).sync <= (r.victory_points_threshold ?? 0) : this.opponent(p).points >= (r.victory_points_threshold ?? 0); return condition && this.canAct(p, r); }
